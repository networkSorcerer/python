# 접근 제한자 란? 클래스 내부에 선언된 변수에 대한 접근을 제한 하는 것
# 내부 변수를 외부 에서 접근 하고자 할때 지정된 게터와 세터 메소드 를 통해서 만 접근 하도록 하는 것
# private(__) : 같은 클래스 내 에서만 접근 가능
# protected(_) : 같은 클래스 내 이거나 상속 관계가 존재 하면 접근 가능
# public (변수에 아무 것도 안 붙임 ) : 아무 데서나 접근 가능
